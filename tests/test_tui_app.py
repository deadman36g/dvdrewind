    ...
    async def test_main_layout_is_three_column_and_quiet_footer(self):
        with patch("src.tui_app._fetch_status", return_value=SAMPLE_STATUS), patch(
            ...
            async with app.run_test(size=(140, 50)) as pilot:
                ...
                self.assertIsNotNone(app.query_one("#health_errors", Static))
                self.assertIsNotNone(app.query_one("#activity_graph", Static))
                self.assertIsNotNone(app.query_one("#success_graph", Static))
                self.assertIn("COLLECTION", str(app.query_one("#left_stats", Static).content))
                footer = str(app.query_one("#footer_keys", Static).content)
                self.assertIn("Navigate", footer)
                self.assertIn("Switch panels", footer)
    ...
    async def test_repair_sections_expose_real_queues_and_actions(self):
        with patch("src.tui_app._fetch_status", return_value=SAMPLE_STATUS), patch(
            ...
            async with app.run_test(size=(150, 54)) as pilot:
                ...
                imdb_cards = "\n".join(str(app.query_one(selector, Static).content) for selector in ("#health_imdb", "#health_art", "#health_complete", "#health_errors"))
                self.assertIn("MATCHED", imdb_cards)
                self.assertIn("UNMATCHED", imdb_cards)
                self.assertIn("ERRORS", imdb_cards)
                self.assertIn("REMAINING", imdb_cards)
                self.assertIn("TITLE DETAILS", str(app.query_one("#best_next", Static).content))
                self.assertIn("LAST REPAIR RESULT", str(app.query_one("#catalog_mix", Static).content))
                await pilot.press("a")
                await pilot.pause()
